...
class Forget_Password_Form(forms.ModelForm):
    ...
    def clean_email(self):
        ...
        if User.objects.filter(email=email).exists():
            ...
            raise forms.ValidationError("This email does not exist enter valid email.")
    ...
    class Meta:
        ...
        widgets = {
            'user_image': forms.FileInput(attrs={
                ...
                'data-parsley-required-message': "Select profile picture",
                'accept':'.jpg, .jpeg',
                "id":"imageupload"
            })
        }
...
class ShippingForms(forms.ModelForm):
    state_choices_from_db = [(state.state, state.state) for state in State.objects.all()]
    STATE_CHOICES = [('Select State', 'Select State')] + state_choices_from_db 
