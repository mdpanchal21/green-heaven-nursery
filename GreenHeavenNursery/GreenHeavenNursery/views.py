...
from .forms import Custuserform,ShippingForms,ProfilePictureForm
...
def search_product(request):
    ...
    if query:
        ...
        results = SubCategory.objects.filter(title__icontains=query)
        subcatresults = [{'name': category.title,'id':category.id,'sid':category.category.id} for category in results]
        
        # Query products based on the search query
        results = Product.objects.filter(title__icontains=query)
        proresults = [{'name': product.title,'id':product.id} for product in results]
        
        print(proresults,catresults,subcatresults)
        return JsonResponse({'results3':subcatresults,'results1': catresults,'results2':proresults})
